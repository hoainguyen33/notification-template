...
type RequestOtpService interface {
...
type requestOtpService struct {
	RequestOtpRepository repository.RequestOtpRepository
	VerifyOtpRepository  repository.VerifyOtpRepository
	//GetcarePhoneService  GetcarePhoneService
}

func NewRequestOtpService(requestOtpRepository repository.RequestOtpRepository, verifyOtpRepository repository.VerifyOtpRepository,
...
func (ro *requestOtpService) RequestOtpAdd(requestOtpAdd model.RequestOtpAdd) error {
	code := GeneratorNumber(6)
	...
	// err := ro.GetcarePhoneService.NewMessageWithoutUser(phoneSMS)
	// if err != nil {
	// 	return err
	// }
...
func (ro *requestOtpService) RequestOTP(requestOtpAdd model.RequestOtpAdd, code string) error {
	...
	if _, _, err := ro.RequestOtpRepository.Create(requestOTP); err != nil {
		return err
	}
	return nil
}

func (ro *requestOtpService) CheckRequestOTP(phone string) error {
	today := time.Now()
...
// can chinh sua lai
func (ro *requestOtpService) VerifyOTP(param model.VerifyOTPParam) error {
	code := param.Code
	...
	where = fmt.Sprintf("request_otp_id=%s", records[0].ID)
